...
def prepare_fit(kic=10935310):
    info   = get_eb_info(kic,L3=0.027289778)
    fitdict = get_data()
    return fitdict,info


def do_model_fit(clobber=False,nwalkers=100,burnsteps=100,mcmcsteps=100,
                 fitperiod=False,fitlimb=False,network=None):
    ...
    ecc = 0.01
    omega = 45d * !dpi/ 180d ;radians
    inc = 88d                ;degrees
    
    period = 4.128795261d         ;days
    t0 = 2454957.322093d          ;BJD
    
    int_time = 60d / (24d * 3600d) ;60 seconds converted to days
    
    ;LD params (Kipping q's)
    q1_p = 0.5d
    q2_p = 0.5d
    q1_s = 0.5d
    q2_s = 0.5d
    
    f2_f1 = 0.5d ;surface brightness ratio, total flux ratio = f2_f1 * (r2_r1)^2
    
    third_light = 0.027289778d ;fraction of total flux
    
    time = t0 + dindgen(10000)*period/10000d
    """
    
    info   = get_model_info(L3=0.027289778,network=network)
    fitdict = get_model_data()
    lnprob,chains,variables = all_fit(fitdict,info,reduce=1,fit_L3=False,
                                      nwalkers=nwalkers,burnsteps=burnsteps,
                                      mcmcsteps=mcmcsteps,clobber=clobber,
                                      fit_period=fitperiod,fit_limb=fitlimb,network=network)
    bestvals = best_vals(fitdict,info,chains=chains,lp=lnprob) 
    params_of_interest(fitdict,chains=chains,lp=lnprob)
...
def make_model_data(m1=0.7,m2=0.35,r1=0.7,r2=0.5,ecc=0.02,omega=30.0,inc=88.5,
    ...
    massratio = m2/m1 if gravitydark else 0.0

    

    fitlighttravel = usereflection = True
    ...
    Mstar1 = m1*c.Msun
    Mstar2 = m2*c.Msun
    sma = ((period*24.0*3600.0)**2*c.G*(Mstar1+Mstar2)/(4.0*np.pi**2))**(1.0/3.0)
    ...
    p0_3  = np.cos(np.radians(inc)) # cos i
    ...
    ebpar0 = {'J':J, 'Rsum_a':(r1*c.Rsun + r2*c.Rsun)/sma, 'Rratio':r2/r1,
              ...
              'ecosw':ecosw0, 'esinw':esinw0, 'Period':period, 't01':0.0, 't02':period/2., 
              'et01':0.0, 'et02':period/2.0, 'dt12':period/2.0, 'tdur1':2.0, 'tdur2':2.0, 
...
#    variables =["J","Rsum","Rratio","cosi","ecosw","esinw","magoff","t0"]
    ...
    parm,vder = vec_to_params(p0_init)

    # need vector t for which output model will be evaluated.
    t = np.linspace(-1*period/4.0,3*period/4,10000)
    
    lightmodel = compute_eclipse(t,parm,modelfac=11.0,fitrvs=False,tref=0.0,
    ...
    rvs = compute_eclipse(t,parm,modelfac=11.0,fitrvs=True,tref=0.0,
                          period=period,ooe1fit=None,ooe2fit=None,unsmooth=False)
    ...
    lout = np.array([t+2454833.0,lightmodel])
    np.savetxt('lightcurve_model.txt',lout.T)

    r1out = np.array([t+2454833.0,rv1])
    np.savetxt('rv1_model.txt',r1out.T)

    r2out = np.array([t+2454833.0,rv2])
...
def check_model():
    t1,rv1 = np.loadtxt('rv1_model.txt',unpack=True)
    t2,rv2 = np.loadtxt('rv2_model.txt',unpack=True)

    time,light = np.loadtxt('lightcurve_model.txt',unpack=True)

    plt.plot(time,light,'.')
    
    plt.figure(2)
    plt.plot(t1,rv1,t2,rv2,'.')

    
def get_data():
    global rvdata1, rvdata2, vsys

    file = 'kepler_detrended_psm_for_js.idl'
    
    dict = readsav(file,python_dict=True)

    time = dict['time_flux']-2454833.0
    flux = dict['flux']
    eflux = dict['err_flux']

    rvdata1 = np.array([dict['t1']-bjd,dict['rv1'],dict['e_rv1']]).T
    rvdata2 = np.array([dict['t2']-bjd,dict['rv2'],dict['e_rv2']]).T

    maxval = max(np.max(rvdata1[:,1]),np.max(rvdata2[:,1]))
    minval = min(np.min(rvdata1[:,1]),np.min(rvdata2[:,1]))
    vsys = (maxval+minval)/2.0

    return {'time':time,'flux':flux,'eflux':eflux}
...
def get_model_data():
    ...
    file = 'psm_model_eb.idl'
    
    dict = readsav(file,python_dict=True)

    # need errors and 
    time = dict['time']-2454833.0
    flux = dict['flux']
    eflux = np.zeros(len(flux))+0.0001

    e_rv1 = np.zeros(len(dict['rv1']))+0.01
    e_rv2 = np.zeros(len(dict['rv2']))+0.01
    
    rvdata1 = np.array([time,dict['rv1'],e_rv1]).T
    rvdata2 = np.array([time,dict['rv2'],e_rv2]).T
...
def get_model_info(limbmodel='quad',errfac=3,network=None,L3=0.0):
...
# Star properties
    ...
    test = os.path.exists(path+'RVs/'+name+'_rvfitparams.txt')
    if test == True:
        info2 = np.loadtxt(path+'RVs/'+name+'_rvfitparams.txt')
...
# Eclipse information (From Phil's email)
    period0 = 4.128795261
    eperiod0 = 0.001
    ephem1 = 2454957.322093
    eephem1 = 0.1
    ephem2 = 2454959.3864906305
    eephem2 = 0.1
    dur1 = 2.5/24.0
    dur2 = 2.5/24.0
    ecosw0 = 0.001
...
# Read in RV data
    try:
        rvdata = get_rv_data(0,network=network)
        vsys = rvdata['vsys']
    except:
        vsys = 0.0
